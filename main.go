...
func initDB() {
	var err error
	db, err = gorm.Open("postgres", "host=localhost port=5432 user=postgres dbname=Golang sslmode=disable password=shivang@1206")
	db1, err = gorm.Open("mysql", "shivang:shivang@tcp(localhost:3306)/golang1?parseTime=True")
	if err != nil {
		fmt.Println(err)
